...
fn test_search_by_rarity() {
    // existing implementation from jay branch
}
...
fn test_nft_without_creator_defaults_to_none() {
    // existing implementation from main branch
}
    let env = setup_env();
    let client = NftRewardClient::new(&env, &env.register_contract(None, NftReward));
...
fn test_search_nfts_multiple_filters() {
    ...
    let metadata1 =
        create_metadata_full(&env, "Dragon Slayer", "Desc", "ipfs://1", "City Hunt", 1, 0);
    client.mint_reward_nft(&1, &player, &metadata1);

    let metadata2 =
        create_metadata_full(&env, "Dragon Master", "Desc", "ipfs://2", "Forest Hunt", 1, 1);
    client.mint_reward_nft(&2, &player, &metadata2);

    let metadata3 =
        create_metadata_full(&env, "Phoenix Rider", "Desc", "ipfs://3", "City Hunt", 3, 0);
    client.mint_reward_nft(&3, &player, &metadata3);
...
fn test_mint_from_map_creator_defaults_to_player() {
    ...
    metadata.set(
        Symbol::new(&env, "title"),
        String::from_str(&env, "Default Creator"),
    );
...
fn test_burn_removes_nft_and_clears_owner_list() {
    ...
    let owner = Address::generate(&env);
    let metadata = create_metadata(&env, "Burnable NFT", "Desc", "ipfs://burn");
...
fn test_search_nfts_empty_results() {
    ...
    // Search with non-matching title filter
    let results = client.search_nfts(
    ...
    // Search with non-matching rarity filter
    let results = client.search_nfts(
        None,
        None,
        Some(5),
...
#[test]
fn test_mint_reward_nft_from_map_with_missing_keys_uses_defaults() {
    use soroban_sdk::{Map, Symbol, Val, IntoVal};
    ...
    assert_eq!(nft.metadata.image_uri, String::from_str(&env, ""));
    assert_eq!(
...
fn test_update_metadata_doesnt_duplicate_nft_ids() {
    ...
    let metadata =
        create_metadata(&env, "Original Title", "Original Desc", "ipfs://original");
    let nft_id = client.mint_reward_nft(&1, &player, &metadata);
