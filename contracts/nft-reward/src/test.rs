...
fn test_mint_reward_nft_rejects_empty_image_uri_consistently() {
    ...
    let direct_err = client
        ...
        .unwrap_err();
    assert_eq!(direct_err, Ok(NftErrorCode::InvalidMetadata.into()));
...
fn test_mint_reward_nft_enforces_single_uri_length_limit() {
    ...
    let err = client
        ...
        .unwrap_err();
    assert_eq!(err, Ok(NftErrorCode::InvalidMetadata.into()));
}
...
fn test_mint_reward_nft_from_map_present_wrong_type_returns_invalid_metadata() {
    ...
    metadata2.set(
        Symbol::new(&env, "image_uri"),
        123u32.into_val(&env), // u32, not String
    );

    let res2 = client.try_mint_reward_nft_from_map(&player, &1, &player, &metadata2);
    assert_eq!(
        res2,
        Err(Ok(NftErrorCode::InvalidMetadata)),
...
fn test_initialize_emits_event_with_admin_minter_max_supply() {
    ...
    let init_events: Vec<_> = events
        ...
        .filter(|(_, topics, _)| {
            !topics.is_empty()
                && topics.get(0).unwrap().to_xdr(&env) == Symbol::new(&env, "INIT").to_xdr(&env)
        })
        .collect();

    // Should have at least one INIT event
    assert!(!init_events.is_empty(), "No INIT event found");
}
...
fn test_add_authorized_contract_emits_event() {
    ...
    let auth_events: Vec<_> = events
        ...
        .filter(|(_, topics, _)| {
            !topics.is_empty()
                && topics.get(0).unwrap().to_xdr(&env) == Symbol::new(&env, "AUTH_ADD").to_xdr(&env)
        })
        .collect();

    // Should have at least one AUTH_ADD event
    assert!(!auth_events.is_empty(), "No AUTH_ADD event found");
}
...
fn test_remove_authorized_contract_emits_event() {
    ...
    let auth_events: Vec<_> = events
        ...
        .filter(|(_, topics, _)| {
            !topics.is_empty()
                && topics.get(0).unwrap().to_xdr(&env) == Symbol::new(&env, "AUTH_REM").to_xdr(&env)
        })
        .collect();

    // Should have at least one AUTH_REM event
    assert!(!auth_events.is_empty(), "No AUTH_REM event found");
}
...
fn test_set_reward_manager_emits_event() {
    ...
    let reward_events: Vec<_> = events
        ...
        .filter(|(_, topics, _)| {
            !topics.is_empty()
                && topics.get(0).unwrap().to_xdr(&env) == Symbol::new(&env, "RWD_MGR").to_xdr(&env)
        })
        .collect();

    // Should have at least one RWD_MGR event
    assert!(!reward_events.is_empty(), "No RWD_MGR event found");
}
...
fn test_add_authorized_contract_requires_admin_authorization() {
    ...
    if result.is_ok() {
        ...
        let auth_events: Vec<_> = events
            ...
            .filter(|(_, topics, _)| {
                topics.len() > 1
                    && topics.get(0).unwrap().to_xdr(&env)
                        == Symbol::new(&env, "AUTH_ADD").to_xdr(&env)
            })
            .collect();
        ...
        assert!(
            !auth_events.is_empty() || result.is_err(),
...
fn test_admin_update_image_uris_paginates_across_multiple_calls() {
    ...
    for i in 0..n {
        let nft = client.get_nft(&(i as u64 + 1)).unwrap();
...
#[test]
fn test_unauthorized_cannot_mint_after_init() {
    let env = setup_env();
    let contract_id = env.register_contract(None, NftReward);
    let client = NftRewardClient::new(&env, &contract_id);
    ...
    // After initialization: an unauthorized address should not be able to mint
    let post_init = client.try_mint_reward_nft(&arbitrary, &1, &player, &metadata);
    assert_eq!(post_init, Err(Ok(NftErrorCode::Unauthorized.into())));
}
...
fn test_completion_rank_is_distinct_per_player() {
    ...
    let (_, _, data1): (Address, soroban_sdk::Vec<Val>, Val) =
        events1.get(events1.len() - 1).unwrap();
    ...
    assert_eq!(ev2.completion_rank, 5, "second player should have rank 5");
    assert_ne!(
...
fn test_mint_rejects_royalty_bps_above_max() {
    ...
    let err = client
        ...
        .unwrap_err();
    assert_eq!(err, Ok(NftErrorCode::InvalidRoyalty.into()));
}
